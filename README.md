...
Если у тебя wifi, его нужно подключить. Введи команду:
```bash
nmtui
```
Выбери пункт "**Activate a connection**".
Выбери свою сеть из списка и введи пароль.
Выйди (Quit) и тоже пропингуй для верности.
## Славянская сказка
